...
export async function POST(request) {
  ...
  try {
    const res = await fetch(`${bridgeUrl}/api/public/report`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        guildId: body.guildId,
        reporterId: session.user.discordId, // jamais fourni par le client, toujours la vraie session
        reason: body.reason,
    ...
    console.error('❌ /api/report a échoué:', err);
